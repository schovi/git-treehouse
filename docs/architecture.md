...
| `tui/commands.go` | Async `tea.Cmd` builders for refresh, enrichment, GitHub, size, config, and external actions. |
| `gitdata/load.go` | Orchestrates the git calls that build the worktree/branch model. It detects the Git version once at startup and carries it through TUI reloads; Git older than 2.41 skips unsupported branch metadata instead of running per-branch compatibility queries. Status enrichment also runs `git diff --numstat HEAD` for `Worktree.ChangedFiles`; `enrichContextGraphs` runs per non-main worktree row, for `Worktree.Graph`, three `git log` calls (commits ahead of main, behind main, and on the upstream via `HEAD..@{u}`) plus `git merge-base` and a deeper `git log` from it for the fork point and the shared ancestors that pad the Git context frame to the paired left column's height; `LoadBranchContextGraph` does the same for `Branch.Graph` on a branch-only row, but runs from the repo root against the branch ref (`refs/heads/<name>` and `<name>@{u}`) since there is no checkout to log from. It is loaded lazily for the selected branch (the TUI's `selectedBranchGraphCommand`, like the PR-review and full-size loads) rather than eagerly for every branch, because a repo can have many local branches and only the selected one is shown. Both share `loadContextGraph`. |
...
| `tui/frames.go` | Auxiliary context frames below the Details panel (PR review, Changes, Git context). Pure renderers returning a bordered section box or `""`. `model.detailBlocks` composes the below-Worktrees region as two columns: the left column stacks Details + Changes + PR review (via `lipgloss.JoinVertical`), the right is the Git context frame grown to match the left column's height; the two are placed side by side with `lipgloss.JoinHorizontal` on wide panels and stacked otherwise. The Git context renderer takes a `graphSource` (built from either a worktree row or a branch-only row) so the same frame serves both row kinds. The resulting block list is sized into the table height budget by `availableTableHeightForBlocks`. |
| `github/review.go` | `LoadPullRequestReview` (`gh pr view --json`) + pure `ParsePullRequestReview`/`ParseChecks` for the PR review frame. The TUI loads it lazily for the selected row and caches it by PR number (storing failed attempts too, so the frame's loading state resolves). |
