...
- **Graceful degradation:** without `--cd-file`, a non-TTY stdout receives the selected path, so `cd (git-treehouse)` works bare. With interactive stdout, the selected path is not printed; instead the app writes a `gth` shell-integration hint to stderr.
