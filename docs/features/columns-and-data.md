...
4. **No `git fetch` on startup.** Ahead/behind reflects the last fetch. The TUI reloads local state every 30 seconds while idle; automatic reloads always refresh status and changed-file counts, reuse a worktree's Git context graph only when its path and `HEAD`, plus the main worktree `HEAD`, are unchanged, and reuse Git-aware size, full size, and disk breakdown when that worktree's path and `HEAD` are unchanged. This has no TTL, so generated files can stay stale until `r`. `r` triggers `git fetch --prune` and bypasses all local enrichment reuse before swapping the table so the existing rows stay visible during refresh. Manual fetches disable Git terminal prompts and use SSH batch mode unless the user already set `GIT_SSH_COMMAND`; all reload work times out after two minutes. A failed manual refresh clears its spinner and shows the error, while auto-refresh remains quiet and can retry later.
